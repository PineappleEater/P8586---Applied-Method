...
libname a "C:\Users\yh2736\OneDrive - cumc.columbia.edu\Desktop\today";
...
*(1-2) call in macro;
%let PATH = C:\Users\yh2736\OneDrive - cumc.columbia.edu\Desktop\today;
...
%let COVARS   =  age_59  age_cat3_: RACE1 RACE_: INSU_: Income_: CDCC_: stage2_: grade2_: facility_location_: 
                facility_type_: year_of_diagnosis_cat_:;
...
data ps_IPTW1;
set ps_no_interaction;
if surgery=1 then IPTW=1/propensity_score;
else if surgery=0 then IPTW=1/(1-propensity_score);
run;
...
*(2)Check PS and IPTW weights distribution;
proc sort data = ps_IPTW1; by surgery;run;
proc print data=ps_IPTW1 noobs;
var surgery propensity_score IPTW;
run;/*export to excel sheet for scatter plots */

*(3)Create PS-IPTW table 1;
...
*(4) PS IPTW outcome model;
proc surveylogistic data=ps_IPTW;
format surgery surgery.;
class Surgery (ref='No')/param=ref;
model death (event='1') =  Surgery;
weight IPTW;
run; /*aOR = 0.542 95%CI: 0.384-0.764*/
proc genmod data=ps_IPTW;
format surgery surgery.;
class Surgery (ref='No')/param=ref;
model death (event='1') =  Surgery/dist=poisson link=log;
weight IPTW;
estimate "Surgery No"    surgery 0/exp;
estimate "Surgery Yes"   surgery 1/exp;
run; /*aRR=0.595 95%CI: 0.487-0.728 */
...
*(3) Distribution of selected characteristics by quintile;
...
proc genmod data=A.ps_no_interaction_2;
 format surgery surgery.;
 class surgery (ref='No')/param=ref;
model death (event='1') =  surgery propensity_score propensity_score*propensity_score;
estimate "Surgery No"    surgery 0/exp;

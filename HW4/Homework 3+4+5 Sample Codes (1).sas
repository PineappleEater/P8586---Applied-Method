...
libname a "E:\Shared_Point\OneDrive - Columbia University Irving Medical Center\Desktop\Today_P8586";
...
*(1-2) call in macro;
%let PATH = E:\Shared_Point\OneDrive - Columbia University Irving Medical Center\Desktop\Today_P8586;
...
proc surveylogistic data=ps_IPTW1_ATT;
format surgery surgery.;
class Surgery (ref='No')/param=ref;
model death (event='1') =  Surgery;
weight ATT_IPTW;
run; /*aOR = 0.541 95%CI: 0.377-0.775 */ 
data ps_IPTW1;set ps_IPTW1;pat_id = _n_;run; 
...
ods graphics on;
proc psmatch data = UTERINE_CA1288_model2 region=allobs;
  class &char. surgery;
  psmodel surgery (treated='1')= &char.;
  psweight weight=wt_ate / treat=surgery estimand=ATE;
  assess lps var=(&char.)/plots=all weight=wt_ate;/*lps = logit of propensity score*/
  output out(obs=match)=OutEx1;
run;
ods graphics off;
proc print data = OutEx1 (obs=20);run;
